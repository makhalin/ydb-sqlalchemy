...
class TestAsTable(TablesTest):
    ...
    def test_upsert_as_table(self, connection):
        ...
        upsert_stm = ydb_sa.upsert(tb).from_select(
            ["id", "val_int", "val_str"],
            sa.select(
    ...
    def test_insert_as_table(self, connection):
        ...
        insert_stm = sa.insert(tb).from_select(
            ["id", "val_int", "val_str"],
            sa.select(
                sa.column("id", type_=Integer), sa.column("val_int", type_=Integer), sa.column("val_str", type_=String)
            ).select_from(sa.func.AS_TABLE(bind_param)),
        ...
        rows = connection.execute(sa.select(tb).where(tb.c.id >= 4).order_by(tb.c.id)).fetchall()
        assert rows == [
            (4, 40, "d"),
            (5, None, "e"),
        ]

    def test_upsert_from_table_reflection(self, connection, metadata):
        table = Table(
            "test_upsert_from_table_reflection",
            metadata,
            Column("id", Integer, primary_key=True),
            Column("val_int", Integer, nullable=True),
            Column("val_str", String, nullable=True),
